class User
  ...
  def change_email(company, new_email)
    ...
    @email = new_email
    @type = new_type
    @email_changed_events.add(EmailChangedEvent.new(user_id=@user_id, new_email=new_email))
  end
end

class EmailChangedEvent
  def initialize(user_id:, new_email:)
    @user_id = user_id
    @new_email = new_email
  end
end
...
class UserController
  def change_email(user_id, new_email)
    ...
    user.email_changed_events.each do |email_changed_event|
      MessageBus.send_email_changed_message(email_changed_event.user_id, email_changed_event.new_email)
    end
  end
end
